...
export default function ResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { correctCount, percentage } = calculateScore(answers, quiz.questions.length);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="space-y-6 pt-8">
        <h3 className="text-xl font-bold text-slate-800 border-b pb-4">Review Jawaban</h3>
        ...
        {quiz.questions.map((question, idx) => {
          ...
          return (
            <div key={question.id} className="bg-[var(--color-surface)] p-6 rounded-xl border border-slate-200">
              ...
              <div className="pl-12 space-y-4">
                <div className="grid gap-2">
                  <div className="flex gap-2 items-start text-sm">
                    <span className="font-medium text-slate-500 w-32">Jawaban Anda:</span>
                    <span className={`font-semibold ${answer.isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                      {answer.selectedKey.toUpperCase()}. {question.options[answer.selectedKey as OptionKey]}
                    </span>
                  </div>
                  {!answer.isCorrect && (
                    <div className="flex gap-2 items-start text-sm">
                      <span className="font-medium text-slate-500 w-32">Jawaban Benar:</span>
                      <span className="font-semibold text-green-600">
                        {question.correctAnswer.toUpperCase()}. {question.options[question.correctAnswer as OptionKey]}
                      </span>
                    </div>
                  )}
                </div>

                <ExplanationPanel explanation={question.explanation} isCorrect={answer.isCorrect} />
              </div>
            </div>
          );
        })}
